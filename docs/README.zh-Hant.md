...
QuotaDeck 是一個自託管網頁面板，用來集中查看 OpenAI Codex 和 Anthropic Claude 的訂閱額度：

- Codex：`Weekly limit`（每週額度）
- Claude：`5h limit`（5 小時額度）和 `Weekly limit`（每週額度）
...
伺服器只保存加密後的憑據。Claude 的 access token 約 1 小時過期，QuotaDeck 會用 refresh token 自動刷新，並把輪換後的新憑據重新加密寫回。預設刷新端點是 Claude Code 目前使用的 `https://platform.claude.com/v1/oauth/token`；如果歷史部署仍把 `CLAUDE_OAUTH_TOKEN_URL` 指向已停用的 `https://console.anthropic.com/v1/oauth/token`，QuotaDeck 會在 404 時自動改用新端點。需要自訂多個備用端點時，可使用逗號分隔的 `CLAUDE_OAUTH_TOKEN_URLS`。
...
儲存後可點「傳送測試」驗證。後台每分鐘（整點）自動刷新所有帳號、保存最新用量，並在額度狀態變化時推播 Bark，即使沒有開啟網頁也能收到提醒。已開啟的頁面會透過伺服器事件自動收到後台刷新後的最新資料，不再依賴固定時間輪詢；設 `BACKGROUND_REFRESH=0` 可關閉後台刷新。

Bark 推播會把帳號名稱和額度狀態傳送到你配置的 Bark 伺服器，請確認信任該伺服器。

## 唯讀 API 介面（無頭用戶端）

無頭用戶端（如 tmux 狀態列、指令稿）可以用唯讀 API token 直接拉取額度快照，免登入、免 cookie。

在 `.env` 裡設定：

```bash
READ_API_TOKEN=$(openssl rand -base64 32)   # 一長串隨機密鑰
READ_API_USERNAME=your-username             # token 對應的使用者名稱
```

之後用 `Authorization: Bearer <token>` 或 `X-API-Key: <token>` 請求：

```bash
curl -H "Authorization: Bearer $READ_API_TOKEN" https://your-domain/api/limits
```

回傳的就是後台保存的最新快照（`{ "results": [...] }`，結構同網頁用的介面）。網頁首次載入以及 tmux 等需要本輪在線結果的用戶端使用 `GET /api/limits?refresh=1`；帳號之間會平行查詢，同一帳號的重疊請求會自動合併，回傳後才更新顯示。即時 GET 每個來源位址每小時最多 120 次。

安全說明：

- token **僅對 GET 生效**（`/api/limits`、`/api/accounts` 等唯讀介面），任何寫入操作（匯入、重新整理、改設定）都會回傳 403，cookie 登入不受影響。
- token 等同於該使用者額度資料的唯讀權限，請妥善保管；外洩後改掉 `READ_API_TOKEN` 即失效。
- 留空 `READ_API_TOKEN` 或 `READ_API_USERNAME` 即關閉該功能。
...
- Codex 只顯示伺服器返回的 10080 分鐘 `每週額度`，不再假設 `primary` 或 `secondary` 是固定視窗。
- Claude 顯示 `5 小時額度` 和 `每週額度`。
- 當額度顯示為 `100%` 剩餘時，頁面不顯示重置時間；低於 `100%` 時會顯示重置時間與距離重置還剩多久。
- Codex 的每週額度依伺服器本次返回值顯示。
