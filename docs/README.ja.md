...
QuotaDeck は [Bark](https://github.com/Finb/Bark)（iOS プッシュ通知アプリ）で残量を通知できます。

### クイックスタート

1. iPhone の App Store で **Bark** をインストールします。
2. Bark を開くと、ホーム画面に `https://api.day.app/abcd1234/` のようなアドレスが表示されます。`abcd1234` の部分が **デバイスキー** です（タップでコピー）。
3. QuotaDeck にログイン後、「Bark 通知」を開きます：
   - **Bark サーバー URL**：公式サーバーなら `https://api.day.app`（既定値）。
   - **Bark デバイスキー**：前の手順の `abcd1234` を貼り付けます。
   - **残量しきい値**：残りパーセント、既定 `20`（残り 20% 未満で通知）。
   - 必要な「通知タイミング」にチェックし、一番上の「Bark 通知を有効化」もチェックします。
4. 「保存」→「テスト送信」をクリックします。iPhone に QuotaDeck の通知が届けば成功です。
5. 以降は設定に従って自動で通知されます。手動操作は不要です。

各ユーザーがページの「Bark 通知」で自分の設定を行います：
...
保存後、「テスト送信」で確認できます。サーバーは毎分（正分）に全アカウントを更新して最新の使用量を保存し、状態が変化したときに Bark をプッシュします。ブラウザを開いていなくても通知を受け取れます。開いているページは固定間隔のポーリングではなく、サーバー送信イベントで更新後のスナップショットを自動受信します。`BACKGROUND_REFRESH=0` でバックグラウンド更新を無効化できます。
...
- アカウント名の下にはアカウントのメールアドレスが表示されます（Codex は `auth.json` から解析、Claude はインポート時に取得）。複数アカウントを区別しやすくなります。
- `↻`：このアカウントだけを更新します。
