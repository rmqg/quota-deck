...
- ページには残りパーセントだけを表示します。Codex が返すローリングウィンドウのリセット時刻は、未使用に近い 5 時間枠で「照会時刻 + 5 時間」のように見えて誤解を招くため表示しません。
- Codex が返す使用済み割合が `1%` 以下の場合、ネイティブ TUI の見え方に合わせて `100%` 残りとして表示します。
