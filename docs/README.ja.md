...
保存後、「テスト送信」で確認できます。サーバーは毎分（正分）に全アカウントを更新して最新の使用量を保存し、状態が変化したときに Bark をプッシュします。ブラウザを開いていなくても通知を受け取れます。ページにはこのバックエンドのスナップショットが表示されます。`BACKGROUND_REFRESH=0` でバックグラウンド更新を無効化できます。
